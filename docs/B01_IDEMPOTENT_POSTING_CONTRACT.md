...
新規作成時はStorage実体のbucket、本人prefix、所有者、MIME、サイズを照合する。写真は`photos/<auth.uid>/photos/<uuid>.<ext>`、ラクガキは`rakugakis/<auth.uid>/rakugakis/<uuid>.<ext>`というbucketとpathの組で扱う（pathそのものにbucket名を二重に入れない）。既存Storage policyの本人prefixだけを投稿作成の十分な検証とみなさない。再送でreceiptがある場合は、画像を後に削除していても同じ結果を返す。
